...
def get_helm_registry(config_dp_endpoint):
    # Setting uri
    get_chart_location_url = f"{config_dp_endpoint}/azure-arc-k8sagents/GetLatestHelmPackagePath?api-version=2019-11-01-preview"

    try:
        response = urllib.request.urlopen(
            request.Request(get_chart_location_url, method="POST"))
    except Exception as e:
        raise Exception(f"Failed to get helm registry.{str(e)}")

    try:
        return json.load(response).get('repositoryPath')
    except Exception as e:
        raise Exception(
            f"Error while fetching helm chart registry path from JSON response: {str(e)}"
        )
    ...
    for i in range(retry_count):
        ...
        if response_helm_chart_pull.returncode != 0:
            if i == retry_count - 1:
                raise Exception(
                    (
                        f"Unable to pull {chart_name} helm chart from the registry '{registry_path}': "
...
def export_helm_chart(registry_path, chart_export_path, kube_config, kube_context, helm_client_location, chart_name='azure-arc-k8sagents'):
    ...
    if response_helm_chart_export.returncode != 0:
        raise Exception(
            (
                f"Unable to export {chart_name} helm chart from the registry '{registry_path}': "
...
def get_chart_path(registry_path, kube_config, kube_context, helm_client_location, chart_folder_name='AzureArcCharts', chart_name='azure-arc-k8sagents'):
    ...
    try:
        ...
        logger.warning(
            f"Unable to cleanup the {chart_folder_name} already present on the machine. In case of failure, please cleanup the directory '{chart_export_path}' and try again."
        )
    ...
    if chart_folder_name == Pre_Onboarding_Helm_Charts_Folder_Name:
        ...
        return (
            os.getenv('HELMCHART')
            if os.getenv('HELMCHART')
            else helm_chart_path
        )
...
def install_helm_client():
    ...
    if not os.path.isfile(download_location):
        ...
        if not os.path.exists(download_dir):
            try:
                ...
                raise Exception(f"Failed to create helm directory.{str(e)}")
        ...
        try:
            ...
            raise Exception(f"Failed to download helm client.{str(e)}")
        ...
        try:
            ...
            raise Exception(f"Failed to create helm executable.{str(e)}")
    ...
    if not os.path.isfile(install_location):
        try:
            ...
            raise Exception(f"Failed to extract helm executable.{str(e)}")
...
def helm_install_release(chart_path, subscription_id, kubernetes_distro, kubernetes_infra, resource_group_name, cluster_name,
    ...
    cmd_helm_install = [
        ...
        chart_path,
        "--set",
        f"global.subscriptionId={subscription_id}",
        "--set",
        f"global.kubernetesDistro={kubernetes_distro}",
        "--set",
        f"global.kubernetesInfra={kubernetes_infra}",
        "--set",
        f"global.resourceGroupName={resource_group_name}",
        "--set",
        f"global.resourceName={cluster_name}",
        "--set",
        f"global.location={location}",
        "--set",
        f"global.tenantId={onboarding_tenant_id}",
        "--set",
    ...
    if not no_wait:
        ...
        onboarding_timeout = f"{onboarding_timeout}s"
...
def install_agent():
    ...
    try:
        ...
        raise Exception(f"Failed to parse arguments.{str(e)}")

    try:
        with open(args.privatePem, "r") as f:
            privateKey = f.read()
    except Exception as e:
        raise Exception(f"Failed to get private key.{str(e)}")
